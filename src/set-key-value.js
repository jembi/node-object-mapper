    ...
    if (isValueArray) {
      ...
      if (Array.isArray(fromValue) && _isNextArrayProperty(keys) === false) {
        for (valueIndex = 0; valueIndex < fromValue.length; valueIndex++) {
          ...
          destinationObject[key][arrayIndex + valueIndex] = _setValue(
        ...
        destinationObject[key][arrayIndex] = _setValue(
          destinationObject[key][arrayIndex],
          keys[0],
          keys.slice(1),
          fromValue
        )
      ...
      if (Array.isArray(fromValue)) {
        for (valueIndex = 0; valueIndex < fromValue.length; valueIndex++) {
          value = fromValue[valueIndex]
          destinationObject[arrayIndex + valueIndex] = _setValue(
        ...
        destinationObject[arrayIndex] = _setValue(
          destinationObject[arrayIndex],
          keys[0],
          keys.slice(1),
          fromValue
        )
      ...
      destinationObject[key] = _setValue(
        destinationObject[key],
        keys[0],
        keys.slice(1),
        fromValue
...
function _isEmptyArray(object) {
  return (
    ...
    (object.length === 0 || object.join('').length === 0)
  )
}
